...
public partial class Admin_Accountmaster : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            if (hndlid.Value != "")
            {
                int a = objamd.ProductMaster(Convert.ToInt32(hndlid.Value), txtproductname.Text, Convert.ToDecimal(lbproductamt.Text), Convert.ToDecimal(txtmrp.Text), Convert.ToDecimal(lbdiscount.Text), Convert.ToDecimal(txtbv.Text), Convert.ToDecimal(txtdirect.Text), txtdesc.Text,hndcheque.Value, "U");
                if (a > 0)
                {
            ...
            {
                int a = objamd.ProductMaster(0, txtproductname.Text, Convert.ToDecimal(lbproductamt.Text), Convert.ToDecimal(txtmrp.Text), Convert.ToDecimal(lbdiscount.Text), Convert.ToDecimal(txtbv.Text), Convert.ToDecimal(txtdirect.Text), txtdesc.Text, hndcheque.Value, "N");
                if (a > 0)
                {
